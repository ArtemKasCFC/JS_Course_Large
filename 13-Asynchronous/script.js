...
GOOD LUCK 😀
*/
const renderHTML = function (json) {
  const [data] = json;

  let currencyKey;
  for (let cur of Object.keys(data.currencies)) {
    currencyKey = cur;
  }

  const languages = [];
  for (let lng of Object.values(data.languages)) {
    languages.push(lng);
  }
  languages.splice(2).join(', ');
...
const handleErr = function (response) {
